...
export class Game{
    ...
    initBoard(){
        ...
        this.playerBoardElement.addEventListener("drop", e => {
            ...
            if(this.draggedShip && !this.isGameStarted){
                ...
                if(this.player.canPlaceShip(this.playerBoardElement, row, col, length)){
                    this.player.placeShip(this.playerBoardElement, row, col, length, shipId);
    ...
    attackComputer(row, col) {
        ...
        const computerCell = document.querySelector(`#computerboard .cell[data-row="${row}"][data-col="${col}"]`);
